...
public class Hover : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float skewed = (Mathf.Sin(rawSin * Mathf.PI * 0.5f) + 1f) / 2f; // Skew to favor bottom
        float hoverOffset = skewed * amplitude * (1 - bias);
        localOffset.y = hoverOffset - amplitude * bias;

        transform.localPosition = new Vector3(transform.localPosition.x, localOffset.y, transform.localPosition.z);
    }
}
